...
export default (props) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if(!handleValidation()) {
        setDonationMessage({message: 'Donation form was filled incorrectly', status: 'danger'})
        return false
    }
    ...
    .then(resp => {
        setDonationMessage({message: resp.data.message, status: resp.data.status})
  ...
  return (
          ...
          <div className="donationForm">
            ...
            {donationMessage.message && (
                <Alert variation={donationMessage.status}>{donationMessage.message}</Alert>
            )}
